<?php
	...
	// order 
	$sort_by = Sys::GetP('sort_by', 'cliente_id', true, $prefix);
	$sort_type = Sys::GetP('sort_type', 'ASC', true, $prefix);
	$orders = array(
		"cliente_id"=>array("order_by"=>"cliente_id $sort_type", "sort_type"=>"", "next_sort_type"=>"asc"),
		"cliente_desc"=>array("order_by"=>"cliente_desc $sort_type", "sort_type"=>"", "next_sort_type"=>"asc"),
	...
	$q = new PgQuery("
	SELECT s.*
	FROM public.cliente s
	WHERE 
	(
		s.cliente_id::text ilike '%$filter%'
		OR s.cliente_desc ilike '%$filter%'
		OR s.cliente_numdoc ilike '%$filter%'
	)
	ORDER BY $order_by
	", NULL, false, true);
...
<div style="padding: 3px; height: 26px; line-height: 16px;">
	<span class="bold c-gray fs-12" style="float: left;">Clientes</span>
...
<tr>
	<td class="cell-head bold"><?=renderSort('Id', 'cliente_id')?></td>
	<td class="cell-head bold"><?=renderSort('Descripcion', 'cliente_desc')?></td>
	<td class="cell-head bold c">Num.Doc.</td>
	<td class="cell-head bold r">Info. Adicional</td>
...
<tr class="<?=$prefix?>-tr">
	...
	<td class="cell c"><?=$r['cliente_numdoc']?></td>
	<td class="cell c"><?=$r['cliente_infoadic']?></td>
	<td class="cell">
...
function <?=$prefix?>_new() {
	var w = new ClienteWindow({p_task: 'new'});
	w.show();
};
function <?=$prefix?>_edit(id) {
	var w = new ClienteWindow({p_task: 'edit', p_id: id});
	w.show();
};
...
// Forms
ClienteWindow = Ext.extend(Ext.Window, {
	id:'<?=$prefix?>_window', title: 'Cliente', width: 600, height: 250, modal: true,
	initComponent: function() {
		...
		ClienteWindow.superclass.initComponent.call(this);
	}
});
</script>