	...
	class Registro {  
		...
		public static function UpdatePrestamo() {
            ...
            try {
                ...
                "registro_det_importe"=>-$monto,
                "registro_det_desc"=>'',
                "registro_det_estado"=>'1', // es uno pote efectivamente el dinero salio, muy diferente es la transferencia.
                "usuario"=>Sys::GetUserName(), 
                "end"=>0 
                ));
                $dr->UpdateLogInfo();
        ...
        public static function CancelarPrestamo() {
            ...
            try {
                ...
                $dr->Update();
                // set prestamo ESTADO
                $ri->Set('registro_estado', $r['registro_estado']); // terminado o pendiente
                $ri->UpdateLogInfo();
                $ri->Update();
		...
		public static function UpdateTransferencia() {
            $r = $_POST;
            self::CheckCierreExists(); // si la caja ha sido cerrada ayer y no ha sido aperturada hoy, el cierre no existe segun esta funcion(*).
            // validacion
            $r['registro_det_importe'] = Sys::NUnformat($r['registro_det_importe']);
			...
			if (!self::CheckAperturaExists($r['pventa_id_destino'])) { // si no esta aperturado la caja no se puede transferir
				exit('La CAJA destino no esta aperturada. No es posible realizar la transferencia.');
			}
			
            if (Sys::getTimeStamp($r['registro_fechaentrega']) < Sys::getTimeStamp(date('d/m/Y'))) {
                exit('La fecha de entrega no puede ser menor a la fecha de envio.');
            }
            
            // process
            Sys::BeginTransaction();
            try {
                $diferido = date('d/m/Y')!=$r['registro_fechaentrega']?'1':'0';
                if (trim($r['registro_id_retornar'])=='') {
                    $r['registro_retornar'] = Sys::GetP('registro_retornar', '0'); // si se activa viene 1
                } else {
                    $r['registro_retornar'] = '0';
                }
                ...
                "cliente_id"=>NULL, 
                "registro_desc"=>(trim($r['registro_desc'])!=''?$r['registro_desc'].'.\n':'')."Transferido a $esta_desc_dest - $pventa_desc_dest", 
                "registro_estado"=>'P',
                "registro_interes"=>0,     
                "registro_diferido"=>$diferido,
                "registro_retornar"=>$r['registro_retornar'],
                "end"=>0 
                ));
                $dr->UpdateLogInfo();
                $dr->Update();
                ...
                "usuario"=>Sys::GetUserName(), 
                "registro_fecha"=>$r['registro_fechaentrega'].' '.date('H:i:s'),
                "tipope_id"=>'05', // transferencia
                "cliente_id"=>NULL, 
                ...
                "registro_diferido"=>$diferido,  
                "registro_retornar"=>$r['registro_retornar'], 
                "registro_id_main"=>(trim($r['registro_id_retornar'])!=''?$r['registro_id_retornar']:null),
                "end"=>0 
                ));
                $dr2->UpdateLogInfo();
                $dr2->Update();
        ...
        public static function CheckCierreExists($fecha=NULL, $exit=true) {
            ...
            AND r.tipope_id IN ('99', '01') -- 01: apertura, 99: cierre
            ...
            return ($tipope == '99'?true:false);
        }
		public static function CheckAperturaExists($pventa_id, $fecha=NULL, $exit=false) {
            $fecha = is_null($fecha)?date('d/m/Y'):$fecha;
            $pv_id = $pventa_id;
            $tipope = PgQuery::GetQueryVal("
            SELECT r.tipope_id
            FROM public.registro r 
            WHERE r.registro_estado = 'T' -- terminado
            AND r.tipope_id IN ('99', '01') -- 01: aperturado, 99: cerrado
            AND r.pventa_id = '$pv_id'
            AND (r.registro_fecha::date = '$fecha' OR r.registro_fechacierre = '$fecha')
			ORDER BY r.registro_fecha DESC
            ", '');
            if ($tipope == '01' && $exit == true) {
                exit("La CAJA esta aperturada.");
            }
            return ($tipope == '01'?true:false);
        }
