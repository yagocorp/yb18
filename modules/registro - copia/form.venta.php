<?php
	...
	$module = "registro";
	$prefix = "{$module}_venta";
	// default values
	$id = Registro::GetNextId();
	$r['registro_fecha'] = date('d/m/Y H:i:s');
	$r['cliente_id'] = Sys::GetUserEstablecimientoId()."-0000001";
...
WHERE tc.tipope_id = '04' and tc.establecimiento_id = '".Sys::GetUserEstablecimientoId()."'  
...
<?php
while ($re = $qe->Read()):?>
                    <option value="<?=$re['tipocambio_id']?>" rdata="<?=rawurlencode($qe->GetRowAsJson())?>"><?=$re['moneda_desc_a']." &rArr; ".$re['moneda_desc_de']?></option>
<?php
endwhile; 
?>
                </select>
        ...
        <tr align="left" valign="top">
            <td class="pd">Cantidad <span id="<?=$prefix?>_simbolo_a" class="bold fs-10"><?=$tc_selected['moneda_simbolo_a']?></span></td>
            <td class="pd">
                <input id="<?=$prefix?>_registro_det_importe_a" class="fs-14 r" type="text" name="registro_det_importe_a" value="0" style="width: 150px;"/>
                <span class="c-blue">(entregar)</span>
            </td>
        </tr>
        <tr align="left" valign="top">
        ...
        <tr align="left" valign="top">
            <td class="pd">Importe <span id="<?=$prefix?>_simbolo_de" class="bold fs-10"><?=$tc_selected['moneda_simbolo_de']?></span></td>
            <td class="pd">
                <input id="<?=$prefix?>_registro_det_importe_de" class="fs-14 r" type="text" name="registro_det_importe_de" value="0" style="width: 150px;"/>
                <span class="c-green">(recibir)</span>
            </td>
        </tr>
        <tr align="left" valign="top">
...
<script>	
	...
	$('#<?=$prefix?>_registro_det_importe_a').keyup(function (e) {
        ...
        eval(evalstr);
        v = Math.round(v * 100) / 100; 
        $('#<?=$prefix?>_registro_det_importe_de').val(v);
    }).change(function () { 
        $('#<?=$prefix?>_efectivo').val($('#<?=$prefix?>_registro_det_importe_de').val()); 
    }).keypress(function (e) {
        if (e.keyCode == 13) {
	...
	$('#<?=$prefix?>_factor').keyup(function (e) {
	...
	$('#<?=$prefix?>_efectivo').keyup(function (e) {
		var v = parseFloat($(this).val()) - parseFloat($('#<?=$prefix?>_registro_det_importe_de').val());
		$('#<?=$prefix?>_vuelto').val(v.toFixed(2));
	}).keypress(function (e) {
		if (e.keyCode == 13) {
	...
	function <?=$prefix?>_update() {
		if (confirm('Realmente desea guardar?')) {
			...
			$.post('modules/<?=$module?>/core.php', 'action=UpdateVenta&'+params, function (data) {
				if ($.trim(data)=='ok') {
