<?php
	...
	WHERE r.pventa_id = '".Sys::GetUserPVentaId()."' 
    	AND r.tipope_id = '99' -- cierre de caja
    	AND r.registro_estado = 'T' -- terminado
	ORDER BY r.registro_fecha DESC, r.registro_id DESC
	LIMIT 1
	", NULL, true, true);
    ...
    m.moneda_desc, 
    m.moneda_simbolo,
    SUM(rd.registro_det_importe) as total
    FROM public.moneda m
    LEFT JOIN public.registro r ON r.registro_id = '$uc_id'
    LEFT JOIN public.registro_det rd ON rd.registro_id = r.registro_id AND rd.moneda_id = m.moneda_id 
		AND rd.registro_det_estado = '1' AND rd.clasemov_id = '02' -- no faltante
...
<tr>
    ...
    <td class="cell-head bold r"><input id="<?=$prefix?>_monto_<?=$d['moneda_id']?>" class="<?=$prefix?>-monto r fs-14" type="text" name="registro_det_importe_<?=$d['moneda_id']?>" value="<?=Sys::NFormat(abs($d['total']))?>" style="width: 200px;" maxlength="20"/></td>
</tr>
<?php
	endwhile; 
?>
